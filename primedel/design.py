...
def reverse_complement(seq):
    ...
    rcomp_seq = ''.join(complement[base] for base in reversed(seq))
    return rcomp_seq
...
def onehotencoder(seq):
    ...
    nt = ['A', 'T', 'C', 'G']
    l = len(seq)
    head = [f"{n}{k}" for k in range(l) for n in nt] + [f"{n1}{n2}{k}" for k in range(l-1) for n1 in nt for n2 in nt]
    head_idx = {key: idx for idx, key in enumerate(head)}
    encode = np.zeros(len(head_idx))
    for j in range(l):
        encode[head_idx[f"{seq[j]}{j}"]] = 1.0
    for k in range(l-1):
        encode[head_idx[f"{seq[k:k+2]}{k}"]] = 1.0
    return encode
...
def gen_pegpair(g1,g2,seq,len_homo,p=False,nick_start=None,nick_end=None):
    ...
    if p:
        ...
        return (g1[0], homology1+ins1+pbs1, g1[1], g2[0], homology2+ins2+pbs2, g2[1],
            note, nick_end-nick_start, seq[:nick_start]+'-'*(nick_end-nick_startß)+seq[nick_end:])
    else:
...
def peg_design_by_size(fwd, rev, seq, size_range, homology_length):
    ...
    if min_dsize < 10:
        return 'Minimal deletion size is >10'
    if max_dsize > len(seq) - 100:
        return 'Please reduce your deletion size'  
    pairs = [
        gen_pegpair(a, b, seq, homology_length)
        for a in fwd
        for b in rev
        if a[1] + min_dsize < b[1] < a[1] + max_dsize]
